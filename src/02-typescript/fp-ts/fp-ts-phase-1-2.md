...
> [!info] Почему `import * as O from 'fp-ts/Option'`?
> Во всей главе используется namespace-импорт (`* as O`, `* as E`, `* as TE`). Это стандартное соглашение сообщества fp-ts, которое решает проблему коллизий: несколько модулей экспортируют функции с одинаковыми именами (`map`, `chain`, `filter`). Префикс `O.map` vs `E.map` сразу показывает, для какого типа вызывается операция.
...
Promise.resolve(1).then(n => n * 2) // Promise<number>
```
...
> [!tip] Проверь себя
> Прежде чем двигаться дальше, ответь на вопросы:
> 1. Почему `Promise.then(x => Promise.resolve(x + 1))` -- это пример Monad, а не просто Functor?
> 2. Что вернёт `pipe(O.some(5), O.map(n => O.some(n * 2)))`? Почему это проблема?
> 3. В каких ситуациях нужен `ap` (Applicative), а не `chain` (Monad)?
...
O.fromNullable(null)          // Option<null> -- none
O.fromNullable('hello')       // Option<string> -- some('hello')
O.fromNullable(undefined)     // Option<undefined> -- none
...
E.fromNullable('Value is null')(null)    // Either<string, null> -- left
...
> [!info] `TE.chainEitherK` -- встраивание Either в TaskEither
> Иногда нужно выполнить **синхронную** валидацию (возвращающую `Either`) внутри асинхронного `TaskEither`-pipeline. `TE.chainEitherK` "поднимает" функцию `A => Either<E, B>` в контекст `TaskEither`, избавляя от ручного `TE.fromEither`:
> ```typescript
> // Вместо:
> TE.chain(user => TE.fromEither(validateAge(user)))
> // Пиши:
> TE.chainEitherK(validateAge)
> ```
...
**Усложнение:** добавь функцию `lookupCategory(productId: string): Option<string>`, которая возвращает категорию товара из второй Map. В pipeline получи категорию через `O.chain` (не `O.map` -- почему?).
...
Для `lookupCategory`: `O.chain(id => lookupCategory(id))`. Если бы использовал `O.map`, получил бы `Option<Option<string>>` -- вложенность. `chain` "разворачивает" внутренний `Option`.
...
### Упражнение 3.5: Изоляция side-effects через IO

Перепиши функцию, обернув все побочные эффекты в `IO`/`IOEither`:

```typescript
// До: нечистая функция
function initApp(): void {
  const seed = Math.random()          // side-effect
  const startedAt = Date.now()        // side-effect
  console.log(`App started at ${startedAt}, seed: ${seed}`) // side-effect
}
```

Реализуй через `IO.IO`, `IO.map`, `IO.chain`. Функция `initApp` должна возвращать `IO.IO<void>` и не выполнять ничего при вызове без `()`.

<details>
<summary>Подсказка</summary>

Создай три `IO`: `randomSeed: IO<number>`, `currentTime: IO<number>`, `logMessage: (msg: string) => IO<void>`. Скомбинируй через `pipe` + `IO.chain`. Вызови результат только в конце: `initApp()()`.
</details>
...
> Q: Что делает `TE.chainEitherK` и зачем он нужен?
> A: `TE.chainEitherK` "поднимает" функцию `A => Either<E, B>` в контекст `TaskEither`. Нужен для встраивания синхронной валидации (Either) в асинхронный pipeline (TaskEither). Короткая запись вместо `TE.chain(x => TE.fromEither(f(x)))`.
