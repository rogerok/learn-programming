...
function testVal(description, actual, expectedValue) {
...
function testNothing(description, actual) {
...
function testLeft(description, actual, expectedError) {
  if (actual instanceof Left && actual._value === expectedError) {
    console.log(`  ПРОЙДЕН: ${description}`);
    passed++;
  } else {
    console.error(`  ПРОВАЛЕН: ${description}`);
...
function testIO(description, ioResult, expectedValue) {
