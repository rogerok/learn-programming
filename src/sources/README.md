---
tags: [sources, reference, index]
---

# Useful Links

Коллекция ссылок по frontend-разработке.
Новые разделы: **[Performance](#performance)**, **[Security](#security)**, **[MobX](#mobx)**.

> [!info] Контекст
> Справочный каталог внешних материалов. Для dependency-ordered обучения используй topic MOC, а для очереди чтения — [[READING_LIST|Reading List]].

---

## Содержание

- [JavaScript](#javascript)
- [TypeScript](#typescript)
- [Architecture & Patterns](#architecture)
- [React](#react)
- [MobX](#mobx)
- [Performance](#performance)
- [Security](#security)
- [Next.js](#nextjs)
- [Algorithms & Data Structures](#algorithms)
- [Interview Prep](#interview)
- [CSS](#css)
- [Web & Browser](#web)
- [Backend (Go)](#backend)
- [Infrastructure](#infrastructure)
- [Git](#git)
- [Tools & DevTools](#tools)
- [Linux](#linux)
- [Networks](#networks)
- [Blogs & Resources](#blogs)
- [Courses & Books](#courses)

---

## JavaScript <a id="javascript"></a>

- [JavaScript Visualized: Promise Execution](https://lydiahallie.framer.website/blog/promise-execution)
- [Паттерны реактивности в современном JavaScript](https://habr.com/ru/articles/757770/)
- [Все, что вам нужно знать о Proxy в JavaScript](https://webdevblog.ru/proxy/)
- [Array Grouping in JavaScript: Object.groupBy()](https://dmitripavlutin.com/javascript-array-group/)
- [Let's learn how modern JavaScript frameworks work by building one](https://nolanlawson.com/2023/12/02/lets-learn-how-modern-javascript-frameworks-work-by-building-one/)
- [🧩 Patterns for JavaScript & Node.js — Тимур Шемсединов](https://github.com/tshemsedinov/Patterns-JavaScript)
- [33 Concepts Every JavaScript Developer Should Know](https://github.com/leonardomso/33-js-concepts)
- [Паттерны проектирования JavaScript (книга)](https://habr.com/ru/companies/piter/articles/845702/)
- [JS. Валидация данных. Пишем свой YUP](https://habr.com/ru/articles/800713/)
- [How to parse HTML in JavaScript](https://blog.apify.com/javascript-parse-html/)
- [What Removing Object Properties Tells Us About JavaScript](https://www.smashingmagazine.com/2023/10/removing-object-properties-javascript/)
- [Web Workers в JavaScript: Параллельные вычисления](https://habr.com/ru/articles/767494/)

---

## TypeScript <a id="typescript"></a>

- [TypeScript в деталях (шпаргалка)](https://my-js.org/docs/cheatsheet/mastering-ts/)
- [TypeScript в деталях. Часть 1](https://habr.com/ru/companies/timeweb/articles/685954/)
- [Продвинутые методы использования TypeScript в реальных проектах](https://habr.com/ru/companies/timeweb/articles/906842/)
- [Понять TypeScript c помощью теории множеств](https://habr.com/ru/articles/713800/)
- [Учимся писать сложные TypeScript типы на примере роутинга в React](https://habr.com/ru/articles/735542/)
- [TypeScript: infer и conditional types. Продвинутый TS на примерах](https://habr.com/ru/articles/778190/)
- [Вывод типов с as const и ключевым словом infer](https://habr.com/ru/companies/ruvds/articles/493716/)
- [Type-level программирование в TypeScript: практические кейсы](https://habr.com/ru/articles/871336/)
- [Typescript: принцип подстановки функций](https://habr.com/ru/articles/779458/)
- [Рецепты TypeScript: подстановка параметров в путь](https://habr.com/ru/companies/cloud_ru/articles/860764/)
- [Мощь декораторов TypeScript на живых примерах](https://habr.com/ru/articles/707496/)
- [Learn Advanced TypeScript Types (curry/ramda)](https://medium.com/free-code-camp/typescript-curry-ramda-types-f747e99744ab)
- [Testing Types in TypeScript](https://frontendmasters.com/blog/testing-types-in-typescript/)
- [Mastering Concurrency and Parallelism in TypeScript](https://dev.to/shafayeat/mastering-concurrency-and-parallelism-in-typescript-1bgf)
- [Typesafe Routing with React-Router: Advanced TypeScript](https://iamsahaj.xyz/blog/typesafe-routing-with-react-router/)
- [Intermediate TypeScript: Build Strongly Typed Polymorphic Components](https://www.freecodecamp.org/news/build-strongly-typed-polymorphic-components-with-react-and-typescript/)
- [The Concise TypeScript Book](https://github.com/gibbok/typescript-book)
- [YouTube: Matt Pocock — TypeScript канал](https://www.youtube.com/@mattpocockuk)
- [YouTube: Функциональное программирование на TypeScript (плейлист)](https://www.youtube.com/playlist?list=PLuPevXgCPUIMbCxBEnc1dNwboH6e2ImQo)

---

## Architecture & Patterns <a id="architecture"></a>

- [Явный дизайн в разработке приложений — Беспоясов (серия)](https://bespoyasov.ru/blog/explicit-design-series/)
- [Чистая архитектура фронтенд приложений — Беспоясов](https://bespoyasov.ru/blog/clean-architecture-on-frontend/)
- [Patterns.dev — design patterns и component patterns](https://www.patterns.dev/)
- [Modularizing React Applications with Established UI Patterns — Martin Fowler](https://martinfowler.com/articles/modularizing-react-apps.html)
- [The 20 Essential Principles of Software Development: LoD, SoC, SOLID и другие](https://medium.com/@techievinay01/the-20-essential-principles-of-software-development-lod-soc-solid-and-beyond-6fd50774f994)
- [OCP из SOLID](https://habr.com/ru/articles/917048/)
- [Low Level Design and SOLID Principles](https://dev.to/srishtikprasad/low-level-design-and-solid-principles-4am9)
- [KISS — принцип проектирования](https://habr.com/ru/articles/249639/)
- [Низкая связанность и высокое сцепление](https://habr.com/ru/articles/568216/)
- [Качество кода](https://habr.com/ru/companies/oleg-bunin/articles/433326/)
- [Гайд по написанию и рефакторингу переиспользуемых компонентов — Яндекс](https://habr.com/ru/companies/yandex/articles/662826/)
- [Пишем хорошие компоненты, которые захочется переиспользовать — Авито](https://habr.com/ru/companies/avito/articles/739330/)
- [Tale of a Refactor](https://commerce.nearform.com/blog/2024/tale-of-a-refactor)
- [UI/UX: Учимся использовать настоящий MVC](https://habr.com/ru/articles/893652)
- [Front End System Design Playbook](https://www.greatfrontend.com/front-end-system-design-playbook)

---

## React <a id="react"></a>

- [How React Works — детальный разбор под капотом](https://incepter.github.io/how-react-works/)
- [Decoupling UI and Logic in React: Headless Components](https://itnext.io/decoupling-ui-and-logic-in-react-a-clean-code-approach-with-headless-components-82e46b5820c)
- [React Advanced: Decoupling your components in the right way](https://dev.to/argonauta/react-advance-decoupling-your-components-in-the-right-way-4pkn)
- [Улучшаем дизайн React приложения с помощью Compound components](https://habr.com/ru/companies/alfa/articles/647013/)
- [Попытка создать идеальный компонент формы](https://habr.com/ru/articles/813551/)
- [Оптимизация React: useTransition, useDeferredValue и useOptimistic](https://habr.com/ru/articles/870748/)
- [Wait for pending: A Suspense algorithm exploration](https://dev.to/alexandereardon/wait-for-pending-a-not-great-alternative-suspense-algorithm-1gdl)
- [React Server Components, without a framework?](https://timtech.blog/posts/react-server-components-rsc-no-framework/)
- [The Complete Guide to Mastering File Uploads](https://javascript.plainenglish.io/the-complete-guide-to-mastering-file-uploads-b581e9ee9ee8)
- [Разрабатываем библиотеку компонентов с помощью React и TypeScript](https://habr.com/ru/companies/timeweb/articles/691338/)
- [Пишем продвинутый планировщик: React + Nest + NX](https://habr.com/ru/companies/domclick/articles/672546/)
- [UX для frontend на основе дизайн-принципов MUI](https://habr.com/ru/companies/tbank/articles/883438/)
- [Как PWA помогло клиентам ЮMoney продолжать получать пуши](https://habr.com/ru/companies/yoomoney/articles/880132/)
- [ReactJs Examples — примеры компонентов](https://reactjsexample.com/)
- [Ui kit от Сбера](https://gitverse.ru/sbertech/ui-kit-ce)

---

## MobX <a id="mobx"></a>

- [Как MobX делает объекты реактивными с помощью Proxy](https://habr.com/ru/companies/gnivc/articles/893108/)
- [Под капотом у MobX. Пишем свою реактивную библиотеку с нуля](https://habr.com/ru/articles/689374/)
- [Реализация архитектуры Redux на MobX](https://habr.com/ru/articles/546628/)
- [Отладка и мониторинг в MobX: trace, introspection и spy](https://habr.com/ru/companies/gnivc/articles/855346/)
- [Как организовать API при помощи декларативных конфигов (MobX)](https://github.com/mobxjs-ru/api-by-config/tree/main)
- [Репозиторий с примерами использования MobX](https://github.com/mobxjs-ru)

---

## Performance <a id="performance"></a>

- [Understanding and Improving Largest Contentful Paint (LCP)](https://www.speedcurve.com/web-performance-guide/understanding-and-improving-largest-contentful-paint/)
- [How to Improve Largest Contentful Paint (LCP)?](https://javascript.plainenglish.io/how-to-improve-largest-contentful-paint-lcp-be5d598caf38)
- [Understanding and Improving Cumulative Layout Shift (CLS)](https://www.speedcurve.com/web-performance-guide/understanding-and-improving-cumulative-layout-shift/)
- [How To Improve INP: Yield Patterns](https://kurtextrem.de/posts/improve-inp)
- [How To Improve INP: React](https://kurtextrem.de/posts/improve-inp-react)
- [5 tips to effectively optimize INP in React](https://calendar.perfplanet.com/2024/5-tips-to-effectively-optimize-inp-in-react/)
- [15 neglected page speed optimizations](https://www.speedcurve.com/blog/15-neglected-page-speed-optimizations/)
- [Optimize resource loading with the Fetch Priority API](https://web.dev/articles/fetch-priority)
- [How to make your web page faster before it even loads](https://blog.sentry.io/how-to-make-your-web-page-faster-before-it-even-loads/)
- [Improving rendering performance with CSS content-visibility](https://nolanlawson.com/2024/09/18/improving-rendering-performance-with-css-content-visibility/)

---

## Security <a id="security"></a>

- [Ликбез по распространенным Client-Side уязвимостям](https://habr.com/ru/companies/bastion/articles/757590/)
- [Стреляем себе в ногу из localStorage](https://habr.com/ru/articles/828912/)

---

## Next.js <a id="nextjs"></a>

- [Next.js. Технология современной веб-разработки](https://habr.com/ru/companies/auriga/articles/786912/)
- [How to Optimize Next.js Web Apps for Better Performance](https://www.freecodecamp.org/news/optimize-nextjs-web-apps-for-better-performance/)
- [How to Choose a Deployment Strategy for Your Next.js Application](https://www.freecodecamp.org/news/deployment-strategies-for-nextjs-apps/)
- [Как создать YouTube GIF Maker с Next.js, Node и RabbitMQ](https://habr.com/ru/companies/otus/articles/793624/)
- [Code with Antonio — видео-разработка веб-приложений](https://www.codewithantonio.com/)

---

## Algorithms & Data Structures <a id="algorithms"></a>

- [Learn Data Structures and Algorithms — kirupa.com](https://www.kirupa.com/data_structures_algorithms/index.htm)
- [Алгоритмы и структуры данных на JavaScript — trekhleb](https://github.com/trekhleb/javascript-algorithms)
- [JavaScript: структуры данных и алгоритмы. Часть 1](https://habr.com/ru/companies/timeweb/articles/826424/)
- [Структуры данных для frontend-разработчиков с реальными примерами](https://habr.com/ru/companies/habr_rutube/articles/934130/)
- [9 алгоритмов сортировки и поиска для JS](https://habr.com/ru/companies/simbirsoft/articles/769312/)
- [LeetCode was HARD until I Learned these 15 Patterns](https://blog.algomaster.io/p/15-leetcode-patterns)
- [Leetcode Patterns — seanprashad](https://seanprashad.com/leetcode-patterns/)
- [Самые важные знания для программиста — Тимур Шемсединов](https://github.com/tshemsedinov/Programming-Knowledge/tree/ru)

---

## Interview Prep <a id="interview"></a>

- [BigFrontEnd.dev — Practice + Prepare](https://bigfrontend.dev/)
- [NeetCode — Roadmap по LeetCode](https://neetcode.io/)
- [Задачи с собеседований — pgrammer.com](https://www.pgrammer.com/)
- [Front-end. Вопросы на собеседовании — YauhenKavalchuk](https://github.com/YauhenKavalchuk/interview-questions)

---

## CSS <a id="css"></a>

- [Conditional CSS — ishadeed](https://ishadeed.com/article/conditional-css/)
- [CSS display: contents — ishadeed](https://ishadeed.com/article/display-contents/)
- [CSS-функции min(), max() и clamp()](https://habr.com/ru/companies/ruvds/articles/501634)
- [A Few Ways CSS Is Easier To Write In 2023 — Smashing Magazine](https://www.smashingmagazine.com/2023/11/few-ways-css-easier-write-2023/)
- [Media Queries vs Container Queries – Which Should You Use?](https://www.freecodecamp.org/news/media-queries-vs-container-queries/)
- [How This SCSS Project Stays Organized Starting from a Map](https://dev.to/lovestaco/how-this-scss-project-stays-organized-starting-from-a-map-3phb)
- [Minding the gaps: A new way to draw separators in CSS](https://blogs.windows.com/msedgedev/2025/03/19/minding-the-gaps-a-new-way-to-draw-separators-in-css/)

---

## Web & Browser <a id="web"></a>

- [Как работает браузер](https://habr.com/ru/companies/skillfactory/articles/678400/)
- [How Internet Works — explained from first principles](https://explained-from-first-principles.com/internet/)
- [Рекомендации по формам входа в систему — web.dev](https://web.dev/sign-in-form-best-practices/)

---

## Backend (Go) <a id="backend"></a>

- [Учимся разрабатывать REST API на Go: сокращатель ссылок](https://habr.com/ru/companies/selectel/articles/747738/)
- [REST API with Go, Chi, MySQL and sqlx](https://dev.to/kashifsoofi/rest-api-with-go-chi-mysql-and-sqlx-39fa)
- [Еще один вариант структуры go-приложения](https://habr.com/ru/articles/911248/)
- [Как работать с Postgres в Go: практики, особенности, нюансы](https://habr.com/ru/companies/oleg-bunin/articles/461935/)
- [Go database/sql tutorial](http://go-database-sql.org/)
- [Курс «PostgreSQL для начинающих»: #2 — Простые SELECT](https://habr.com/ru/companies/tensor/articles/780276/)
- [How to build a Telegram Bot with Go + PostgreSQL](https://medium.com/@nikbenlioglu/how-to-build-a-to-do-list-telegram-bot-with-the-golang-postgresql-database-b77b1ec014ba)
- [Best Practices for Designing a Pragmatic RESTful API](https://www.vinaysahni.com/best-practices-for-a-pragmatic-restful-api)
- [Регистрация по взрослому: JWT, UserDetails](https://habr.com/ru/articles/892480/)
- [Аутентификация через ЕСИА: ключевые аспекты интеграции](https://habr.com/ru/articles/893544/)
- [Хороший курс по Go от ВК — Stepik](https://stepik.org/course/187490/syllabus)
- [Backend developer roadmap — oh-my-backend](https://github.com/bzick/oh-my-backend)

---

## Infrastructure <a id="infrastructure"></a>

- [Nginx Tutorial #1: Basic Concepts](https://www.netguru.com/blog/nginx-tutorial-basics-concepts)
- [Как собрать Docker-образ для прода](https://habr.com/ru/articles/917226/)
- [Подробный гайд по деплою Next.js приложения на VPS](https://www.youtube.com/watch?v=sIVL4JMqRfc)

---

## Git <a id="git"></a>

- [Clean Git History, или Тёмная сторона VCS — Ozon](https://habr.com/ru/companies/ozontech/articles/754526/)
- [Become a git guru — Atlassian tutorials](https://www.atlassian.com/git/tutorials)
- [Recovering Deleted Files From Your Git Working Tree](https://www.smashingmagazine.com/2023/12/recovering-deleted-files-git-working-tree/)
- [Чёрт побери, Git!?! — dangitgit.com](https://dangitgit.com/ru)
- [First Aid Git — коллекция часто задаваемых вопросов](https://firstaidgit.ru/)

---

## Tools & DevTools <a id="tools"></a>

- [67 Weird Debugging Tricks Your Browser Doesn't Want You to Know](https://alan.norbauer.com/articles/browser-debugging-tricks)
- [11 хитростей Chrome DevTools](https://habr.com/ru/articles/663904/)
- [11 Useful Frontend Web Developer Web Apps to Boost Productivity](https://javascript.plainenglish.io/11-useful-frontend-web-developer-web-apps-to-boost-productivity-c8b2afeab251)
- [Front-End Tips, Tricks, and Best Practices — phuoc.ng](https://phuoc.ng/collection/tips/)
- [My top JavaScript utilities in just One Line of Code — phuoc.ng](https://phuoc.ng/collection/1-loc/)
- [30 Seconds of Code — short code snippets](https://www.30secondsofcode.org/)
- [Полный гайд по UI-китам](https://habr.com/ru/companies/agima/articles/825080/)
- [How to favicon in 2021 — Evil Martians](https://evilmartians.com/chronicles/how-to-favicon-in-2021-six-files-that-fit-most-needs)
- [A collection of popular CSS layouts — phuoc.ng](https://phuoc.ng/collection/css-layout/)

---

## Linux <a id="linux"></a>

- [85 команд Linux на все случаи жизни](https://lifehacker.ru/komandy-linux/)
- [CMD Challenge — тренажер консоли](https://cmdchallenge.com/)

---

## Networks <a id="networks"></a>

- [Сети для самых маленьких — linkmeup.ru](https://linkmeup.ru/blog/1188/)

---

## Blogs & Resources <a id="blogs"></a>

- [Developer Way — Nadia Makarevich](https://www.developerway.com/)
- [Dmitri Pavlutin — статьи про JavaScript](https://dmitripavlutin.com/)
- [2ality – JavaScript and more — Dr. Axel Rauschmayer](https://2ality.com/index.html)
- [Vladimir Klepov — статьи про React](https://blog.thoughtspile.tech/tags/react/)
- [Саша Беспоясов — архитектура, книги](https://bespoyasov.ru/tags/books/)
- [Игорь Звягин на Хабре — React, хуки, замыкания](https://habr.com/ru/users/igor_zvyagin/)
- [HowProgrammingWorks — Тимур Шемсединов](https://github.com/HowProgrammingWorks/Index)
- [my-js.org — руководства по JS, TS, React, Node.js и другим](https://my-js.org/)
- [Новости 512 — еженедельный фронтенд-дайджест](https://blog.csssr.com/ru/)

---

## Courses & Books <a id="courses"></a>

- [Frontend уровня топовых компаний — karpov.courses](https://karpov.courses/dev/frontend)
- [Кобеза — продвинутые курсы](https://kobezzza.ru/)
- [Fluent React — O'Reilly](https://www.oreilly.com/library/view/fluent-react/9781098138707/)
- [JavaScript Allonge — функциональное программирование](https://leanpub.com/javascriptallongesix/read)
- [Функционально-Лёгкий JavaScript (перевод)](https://github.com/fxzhukov/Functional-Light-JS-RU)
- [The Concise TypeScript Book](https://github.com/gibbok/typescript-book)
- [System Design. Подготовка к сложному интервью](https://habr.com/ru/companies/piter/articles/598791/)
- [Книги для начинающих разработчиков: обзор](https://habr.com/ru/companies/ru_mts/articles/744602/)
