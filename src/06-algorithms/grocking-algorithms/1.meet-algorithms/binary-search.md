---
tags: [cs, algorithms, binary-search]
---

## Бинарный поиск

Бинарный поиск выполняется за **логарифмическое время** т.е. log2n
...
1. Итеративное решение
   ...
   const binarySearch = (list, item) => {
     ...
     while (low <= high) {
       ...
       if (guess > item) {
         // делаем шаг назад в списке, чтобы получить все элементы, которые меньше guess
         high = mid - 1;
       }

       if (guess < item) {
         // делаем шаг вперед в списке, чтобы получить все элементы, которые больше guess
         low = mid + 1;
       }
     }
   };
